...
$\overline x$ | $M, U, L$ | $\overline x \approx (L + M + U)/3$ | `mean = varameta::wan_mean_C3(q_1 = iqr_lower, m = median, q_3 = iqr_higher)`
$\text{se}$ | $M, U, L, n$ | $\text{se} = (U - L)/(2\sqrt{n}\Phi^{-1}((0.75n-0.125)/(n+0.25)))$ | `se = varameta::wan_se_C3(q_1 = iqr_lower, m = median, q_3 = iqr_higher, n = n)`
$s$ | $\text{se}, n$ | $s = \text{se} \times \sqrt{n}$ | `sd = se * sqrt(n)`
$\text{se}$ | $s, n$ | $\text{se} = s / \sqrt{n}$ | `se = sd / sqrt(n)`


## Types of intervention comparisons

## Excluded observations

For odds-ratio the rate and sample size are required for NMA, and for standardised mean difference, the mean, standard deviation, and sample size are required. Where there are missing reported statistics, the observations are excluded from the models.
...
```{r}
obs_excluded %>% 
  select(exclusion_reason, gs_row, outcome, study_id, title, intervention, timepoint, intervention_grouping, mean:change_score) %>% 
